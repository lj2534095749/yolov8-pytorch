...
from nets.evc_block import EVCBlock
...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        ...
        self.evcblock = EVCBlock(
            int(16 * base_channels * deep_mul),  #c1
            int(16 * base_channels * deep_mul), #c2
        ...
        # 768, 80, 80 => 256, 80, 80
        self.conv3_for_upsample2    = C2f(int(base_channels * 16 * deep_mul) + base_channels * 8 + base_channels * 4, base_channels * 4, base_depth, shortcut=False)
        
        # 256, 80, 80 => 256, 40, 40
        self.down_sample1           = Conv(base_channels * 4, base_channels * 4, 3, 2)
        # 512 + 256, 40, 40 => 512, 40, 40
        self.conv3_for_downsample1  = C2f(int(base_channels * 16 * deep_mul) + base_channels * 8 + base_channels * 4, base_channels * 8, base_depth, shortcut=False)
    ...
    def forward(self, x):
        ...
        # 1024 * deep_mul, 20, 20 => 1024 * deep_mul, 20, 20
        evc = self.evcblock(feat3)
        # 1024 * deep_mul, 20, 20 => 1024 * deep_mul, 40, 40
        evc_up2 = self.upsample(evc)
        # 1024 * deep_mul, 40, 40 => 1024 * deep_mul, 80, 80
        evc_up4 = self.upsample(evc_up2)
        
        #------------------------加强特征提取网络------------------------# 
        # 1024 * deep_mul, 40, 40 cat 512, 40, 40 => 1024 * deep_mul + 512, 40, 40
        P4          = torch.cat([evc_up2, feat2], 1)
        # 1024 * deep_mul + 512, 40, 40 => 512, 40, 40
        P4          = self.conv3_for_upsample1(P4)

        # 512, 40, 40 => 512, 80, 80
        P4_upsample = self.upsample(P4)
        # 1024 * deep_mul, 80, 80 cat 512, 80, 80 cat 256, 80, 80 => 1024 * deep_mul + 768, 80, 80
        P3          = torch.cat([evc_up4, P4_upsample, feat1], 1)
        # 1024 * deep_mul + 768, 80, 80 => 256, 80, 80
        P3          = self.conv3_for_upsample2(P3)

        # 256, 80, 80 => 256, 40, 40
        P3_downsample = self.down_sample1(P3)
        # 1024 * deep_mul cat 512, 40, 40 cat 256, 40, 40 => 1024 * deep_mul + 768, 40, 40
        P4 = torch.cat([evc_up2, P3_downsample, P4], 1)
        # 1024 * deep_mul + 768, 40, 40 => 512, 40, 40
