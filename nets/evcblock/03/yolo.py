...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        ...
        # EVCBlock1
        self.evcblock3 = EVCBlock(
            int(16 * base_channels * deep_mul),  #c1
            int(16 * base_channels * deep_mul), #c2
            channel_ratio=4, base_channel=16,
        )
        # EVCBlock2
        self.evcblock2 = EVCBlock(
            int(8 * base_channels * deep_mul),  #c1
            int(8 * base_channels * deep_mul), #c2
            channel_ratio=4, base_channel=16,
        )
        # EVCBlock3
        self.evcblock1 = EVCBlock(
            int(4 * base_channels * deep_mul),  #c1
            int(4 * base_channels * deep_mul), #c2
            channel_ratio=4, base_channel=16,
        )
    ...
    def forward(self, x):
        ...
        F3EVC = self.evcblock3(feat3)
        # 512, 40, 40 => 512, 40, 40
        F2EVC = self.evcblock2(feat2)
        # 256, 80, 80 => 256, 80, 80
        F1EVC = self.evcblock1(feat1)
        
        #------------------------加强特征提取网络------------------------# 
        # 1024 * deep_mul, 20, 20 => 1024 * deep_mul, 40, 40
        P5_upsample = self.upsample(F3EVC)
        # 1024 * deep_mul, 40, 40 cat 512, 40, 40 => 1024 * deep_mul + 512, 40, 40
        P4          = torch.cat([P5_upsample, F2EVC], 1)
        # 1024 * deep_mul + 512, 40, 40 => 512, 40, 40
        ...
        P3          = torch.cat([P4_upsample, F1EVC], 1)
        # 768, 80, 80 => 256, 80, 80
