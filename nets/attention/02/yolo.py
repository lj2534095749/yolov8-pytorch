...
from nets.attention import RNCBAM, EMRNCBAM, CBAM, NAM, EMA, EMCBAM
...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False, att_phi=4):
        ...
        self.attention_block = [RNCBAM, EMRNCBAM, CBAM, NAM, EMCBAM]

        if 0 <= self.att_phi <= 4:
            self.p4Up_att = self.attention_block[self.att_phi](int(base_channels * 8))  # 512
            self.p3Up_att = self.attention_block[self.att_phi](int(base_channels * 4))  # 256
            self.p2Up_att = self.attention_block[self.att_phi](int(base_channels * 2))  # 128

            self.p4Down_att = self.attention_block[self.att_phi](int(base_channels * 8))  # 512
            self.p3Down_att = self.attention_block[self.att_phi](int(base_channels * 4))  # 256
            self.p2Down_att = self.attention_block[self.att_phi](int(base_channels * 2))  # 128
    ...
    def forward(self, x):
        ...
        P4_att = P4.clone()
        if 0 <= self.att_phi <= 4:
            P4_att = self.p4Up_att(P4)

        # 512, 40, 40 => 512, 80, 80
        P4_upsample = self.upsample(P4_att)
        ...
        P3_att = P3.clone()
        if 0 <= self.att_phi <= 4:
            P3_att = self.p3Up_att(P3)

        # 256, 80, 80 => 256, 40, 40
        P3_downsample = self.down_sample1(P3_att)
        ...
        P4_att = P4.clone()
        if 0 <= self.att_phi <= 4:
            P4_att = self.p4Down_att(P4)

        # 512, 40, 40 => 512, 20, 20
        P4_downsample = self.down_sample2(P4_att)
