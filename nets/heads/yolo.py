...
class YoloBody(nn.Module):
    def __init__(self, input_shape, num_classes, phi, pretrained=False):
        ...
        #------------------------加强特征提取网络------------------------# 
        # 128, 256, 512, 1024 * deep_mul
        ch              = [base_channels * 2, base_channels * 4, base_channels * 8, int(base_channels * 16 * deep_mul)]
        self.shape      = None
        self.nl         = len(ch)
        # self.stride     = torch.zeros(self.nl)
        self.stride     = torch.tensor([128 / x.shape[-2] for x in self.backbone.forward(torch.zeros(1, 3, 128, 128))])  # forward
    ...
    def forward(self, x):
        ...
        # 128, 80, 80 cat 256, 80, 80 => 384, 80, 80
        ...
        # P2 128, 160, 160 => num_classes + self.reg_max * 4, 160, 160
