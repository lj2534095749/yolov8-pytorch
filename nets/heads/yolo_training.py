...
class Loss:
    ...
    def bbox_decode(self, anchor_points, pred_dist):
        if self.use_dfl:
            ...
            # DFL的解码
            pred_dist_softmax = pred_dist.view(b, a, 4, c // 4).softmax(3)
            proj_temp = self.proj.to(pred_dist.device).type(pred_dist.dtype)
            pred_dist = torch.cat((pred_dist_softmax[:, 0:a//2, :, :].matmul(proj_temp), pred_dist_softmax[:, a//2:, :, :].matmul(proj_temp)), 1)
