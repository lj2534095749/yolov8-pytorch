...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        # 计算NWD损失
        loss_nwd = wasserstein_loss(pred_bboxes[fg_mask], target_bboxes[fg_mask]).mean().to(pred_dist.device)
...
#             print(self.proj.to(pred_dist.device).type(pred_dist.dtype).shape)
#             ax = torch.zeros((12, 22800, 4, 16)).to(pred_dist.device).type(pred_dist.dtype)
#             bx = torch.zeros(16).to(pred_dist.device).type(pred_dist.dtype)
#             print(ax.shape, bx.shape)
#             print(ax.matmul(bx).shape)
    ...
    def __call__(self, preds, batch):
        ...
        loss[2] *= 1.5  # dfl gain
        loss[3] *= 0.5  # nwd gain
        return loss.sum() # loss(box, cls, dfl, nwd) # * batch_size
